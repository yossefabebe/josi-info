...
import { Award, FileText, Filter, ExternalLink, Download } from 'lucide-react';
...
const Projects: React.FC = () => {
  ...
  // Always show priority certificates regardless of filter
  const priorityCertificates = certificates.filter(cert => cert.alwaysVisible);
  const regularItems = allItems.filter(item => !('alwaysVisible' in item) || !item.alwaysVisible);
  ...
  const filteredRegularItems = activeFilter === 'all' 
    ? regularItems 
    : regularItems.filter(item => item.category === activeFilter);

  // Combine priority certificates with filtered items
  const displayItems = [...priorityCertificates, ...filteredRegularItems];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {displayItems.map((item, index) => {
            ...
            const isPriority = 'alwaysVisible' in item && item.alwaysVisible;
            
            return (
              <div
                key={index}
                className={`bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow group ${isPriority ? 'ring-2 ring-yellow-400 ring-opacity-50' : ''}`}
              >
